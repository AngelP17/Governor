...
import { LoadingState, ApiOfflineState } from "../components/shared/States";
        ...
        {Object.entries(grouped).map(([category, items]) => (
          <section key={category}>
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {items.map((control) => (
                <article key={control.name} className="rounded-2xl border border-line bg-panel/80 p-5 transition hover:border-slate-500/45">
                  ...
                  <div className="mt-4 rounded-xl border border-line bg-slate-950/45 p-3">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-xs text-slate-500">Artifact</p>
                      <CopyButton value={control.artifact} label="Copy" />
                    </div>
                    <p className="mt-1 break-all font-mono text-xs text-slate-200">{control.artifact}</p>
                  </div>
